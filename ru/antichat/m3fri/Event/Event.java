	...
	public void onPlayerChat(cn.nukkit.event.player.PlayerChatEvent event) {
		...
		for (String word : blockedWords) {
			if (message.contains(word.toLowerCase())) {
				...
				String blockedWordsMessage = plugin.getConfig().getString("blockedWordsMessage", "§fВы §e{player} §fнаписал запрещенное слово: §b{word}§r");
		...
		if (matcher.find()) {
			event.setCancelled(true);
			String blockedDomainsMessage = plugin.getConfig().getString("blockedDomainsMessage", "§fВы §e{player} §fнаписал сообщение, в котором содержится доменное имя: §b{domain}§r");
